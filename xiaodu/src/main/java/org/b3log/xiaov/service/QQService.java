...
import com.moltres.util.Property;
import com.moltres.util.SendUrl;
import com.moltres.util.Spider;
...
//    }
     ...
     */
    public void initQQClient(int i) {
        LOGGER.info("开始初始化小薇");

        xiaoV = new SmartQQClient(i, new MessageCallback() {
            @Override
                    ...
                    public void run() {
                        try {
                            ...
                            if (!StringUtils.startsWith(content, key)) { // 不是管理命令，只是普通的私聊
                                // 让小薇进行自我介绍
                                xiaoV.sendMessageToFriend(message.getUserId(), "你好");
            ...
            @Override
            public void onGroupMessage(final int i, final GroupMessage message) {
                new Thread(new Runnable() {
                    @Override
                    public void run() {
                        try {
                            Thread.sleep(500 + RandomUtils.nextInt(1000));
                            onQQGroupMessage(i, message);
                        } catch (final Exception e) {
                            LOGGER.log(Level.ERROR, "XiaoV on group message error", e);
                        }
                    }
            ...
            @Override
            public void onDiscussMessage(final int i, final DiscussMessage message) {
                new Thread(new Runnable() {
                    @Override
                    public void run() {
                        try {
                            Thread.sleep(500 + RandomUtils.nextInt(1000));
                            onQQDiscussMessage(i, message);
                        } catch (final Exception e) {
                            LOGGER.log(Level.ERROR, "XiaoV on group message error", e);
                        }
                    }
        ...
        if (MSG_ACK_ENABLED) { // 如果启用了消息送达确认
            ...
            xiaoVListener = new SmartQQClient(i, new MessageCallback() {
                @Override
                public void onMessage(final Message message) {
                    try {
                        Thread.sleep(500 + RandomUtils.nextInt(1000));
                        ...
                        if (!StringUtils.startsWith(content, key)) { // 不是管理命令
                            // 让小薇的守护进行自我介绍
                            xiaoVListener.sendMessageToFriend(message.getUserId(), "你好");
                ...
                @Override
                public void onGroupMessage(int i, final GroupMessage message) {
                    final String content = message.getContent();
                ...
                @Override
                public void onDiscussMessage(int i, final DiscussMessage message) {
                    final String content = message.getContent();
...
//    }
    ...
    private void sendMessageToGroup(final Long groupId, final String msg) {
        ...
        if (MSG_ACK_ENABLED) { // 如果启用了消息送达确认
            ...
            if (maxRetries == sentTries) {
                LOGGER.info("Pushing [msg=" + msg + "] to QQ qun [" + group.getName() + "]");
                xiaoV.sendMessageToGroup(groupId, "你好");
            }
        }
    }
    ...
    private void sendMessageToDiscuss(final Long discussId, final String msg) {
        ...
        if (MSG_ACK_ENABLED) { // 如果启用了消息送达确认
            ...
            if (maxRetries == sentTries) {
                LOGGER.info("Pushing [msg=" + msg + "] to QQ discuss [" + discuss.getName() + "]");
                xiaoV.sendMessageToDiscuss(discussId, "你好");
            }
        }
    }

    
    public void onQQGroupMessage(int i, final GroupMessage message) {
        ...
        String msg = "";
        if (content.startsWith("淘宝 ") || content.startsWith("淘宝网 "))
        {
        	int index = content.indexOf(" ");
        	String url = null;
			try {
				url = "http://45.78.50.42:8080/" + URLEncoder.encode("淘宝", "UTF-8") + "%20" + URLEncoder.encode(content.substring(index + 1, content.length()), "utf-8");
			} catch (UnsupportedEncodingException e) {
				e.printStackTrace();
			}
        	List<JSONObject> objects = SendUrl.getJSONObject(url);
        	if (objects == null)
        	{
        		sendMessageToGroup(groupId, "啊呀，这次没找到呢");
        		return;
        	}
        	for (int k = 0; k < 5; k++)
        	{
        		...
        		msg = "";
        		msg += "商品:";
        		msg += objects.get(r).getString("title");
        		msg += "  ";
        		msg += "店铺：";
        		msg += objects.get(r).getString("shop");
        		msg += "  ";
        		msg += "最近成交量：";
        		msg += objects.get(r).getString("sales");
        		msg += "  ";
        		msg += "价格：";
        		msg += objects.get(r).getString("price");
        		msg += "  ";
        		msg += " 位置：";
        		msg += objects.get(r).getString("location");
        		msg += "  ";
        		msg += "链接:";
        		msg += objects.get(r).getString("url");
        		sendMessageToGroup(groupId, msg);
        	}
        	return;
        }
        if (content.startsWith("京东 ") || content.startsWith("京东网 "))
        {
        	int index = content.indexOf(" ");
        	String url = null;
			try {
				url = "http://45.78.50.42:8080/" + URLEncoder.encode("京东", "UTF-8") + "%20" + URLEncoder.encode(content.substring(index + 1, content.length()), "utf-8");
			} catch (UnsupportedEncodingException e) {
				e.printStackTrace();
			}
        	List<JSONObject> objects = SendUrl.getJSONObject(url);
        ...
        if (content.startsWith("当当 ") || content.startsWith("当当网 "))
        {
        	int index = content.indexOf(" ");
        	String url = null;
			try {
				url = "http://45.78.50.42:8080/" + URLEncoder.encode("当当", "UTF-8") + "%20" + URLEncoder.encode(content.substring(index + 1, content.length()), "utf-8");
			} catch (UnsupportedEncodingException e) {
				e.printStackTrace();
			}
        	List<JSONObject> objects = SendUrl.getJSONObject(url);
        ...
        if (content.startsWith("苏宁 ") || content.startsWith("苏宁网 "))
        {
        	int index = content.indexOf(" ");
        	String url = null;
			try {
				url = "http://45.78.50.42:8080/" + URLEncoder.encode("苏宁", "UTF-8") + "%20" + URLEncoder.encode(content.substring(index + 1, content.length()), "utf-8");
			} catch (UnsupportedEncodingException e) {
				e.printStackTrace();
			}
        	List<JSONObject> objects = SendUrl.getJSONObject(url);
        ...
        if (content.startsWith("天猫 "))
        {
        	int index = content.indexOf(" ");
        	String url = null;
			try {
				url = "http://45.78.50.42:8080/" + URLEncoder.encode("天猫", "UTF-8") + "%20" + URLEncoder.encode(content.substring(index + 1, content.length()), "utf-8");
			} catch (UnsupportedEncodingException e) {
				e.printStackTrace();
			}
        	List<JSONObject> objects = SendUrl.getJSONObject(url);
        ...
        if (content.equals("今日头条"))
        {
        	...
        	for (int k = 0; k < 9; k++)
        	{
        		msg = "";
        		msg += "标题:";
        		msg += objects.get(k).getString("title");
        		msg += "  ";
        		msg += "链接";
        		msg += objects.get(k).getString("url");
        		sendMessageToGroup(groupId, msg);
        	}
        	return;
        }
        ...
        String answer = Property.getAnswer();
        String[] answers = answer.split(",");
        String keyword = Property.getKeyword();
        String[] keywords = keyword.split(",");
        boolean flag = false;
        int index = 0;
        for (String qu : questions)
        {
        	if (content.contains(URLDecoder.decode(qu)))
        	{
        		sendMessageToGroup(groupId, answers[index]);
        		return;
        	}
        	index++;
        }
        ...
        {
        	if (content.contains(URLDecoder.decode(word)))
        	{
        ...
        if (StringUtils.contains(content, Property.getNames().split(",")[i]) || flag) {
            msg = answer(i, content, userName);
        }
    ...
    public void onQQDiscussMessage(int i, final DiscussMessage message) {
        ...
        String msg = "";
        if (StringUtils.contains(content, Property.getNames().split(",")[i])
    ...
    private String answer(int i,final String content, final String userName) {
       
        String ret = "";
        
            if (Property.getType().split(",")[i].equals("1")) {
                ret = turingQueryService.chat(userName, content);
                ret = StringUtils.replace(ret, "图灵机器人", Property.getNames().split(",")[i] + "机器人");
                ret = StringUtils.replace(ret, "默认机器人", Property.getNames().split(",")[i] + "机器人");

                ret = StringUtils.replace(ret, "<br>", "\n");
            } else if (Property.getType().split(",")[i].equals("2")) {
