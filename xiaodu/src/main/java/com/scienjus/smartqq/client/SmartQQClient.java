...
public class SmartQQClient implements Closeable {
    ...
    //Bot type
    private static final int QQ_BOT_TYPE = 1;

    public SmartQQClient(final int i ,final MessageCallback callback) {
        ...
        if (callback != null) {
            ...
            new Thread(new Runnable() {
                ...
                public void run() {
                    while (true) {
                        if (!pollStarted || !Property.getActive(i)) {
                        	return;
                        }
                        try {
                            pollMessage(i, callback);
    ...
    private void getQRCode(int i) {
        ...
        String filePath;
        try {
            filePath = new File("qrcode" + i + ".png").getCanonicalPath();
        } catch (IOException e) {
    ...
    //登录流程2：校验二维码
    private String verifyQRCode(int i) {
     ...
     */
    private void pollMessage(int j, MessageCallback callback) {
        LOGGER.debug("开始接收消息");
        ...
        for (int i = 0; array != null && i < array.size(); i++) {
            ...
            if ("message".equals(type)) {
                ...
                callback.onGroupMessage(j, new GroupMessage(message.getJSONObject("value")));
            } else if ("discu_message".equals(type)) {
                callback.onDiscussMessage(j, new DiscussMessage(message.getJSONObject("value")));
            }
        }
    }
